...
router.get('artcle_list', async(ctx, next) =>{
    ...
    ctx.render('user/artcle_list', {blogs});
});
...
router.get('updateArtcle', async(ctx, next) =>{
    ...
    //回到编辑页面
    ctx.render('user/editor', {type: blog.type, title: blog.title, mdContent: blog.mdContent, id});
});
...
router.get('previewArtcle', async(ctx, next) =>{
    ...
    ctx.render('user/preview_artcle', {htmlContent: blog.htmlContent});
});
